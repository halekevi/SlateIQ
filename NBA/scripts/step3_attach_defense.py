...
def main():
    ...
    try:
        ...
        d = load_defense_from_db("wnba")
        if d is not None:
            fresh = defense_freshness("wnba")
            print(f"→ Defense loaded from DB ({len(d)} teams, updated {fresh})")
        else:
            print("→ Defense DB empty for WNBA — falling back to CSV")
    ...
    if not key:
        raise RuntimeError(f"❌ Defense data missing team key. Found: {list(d.columns)}")
