...
def main() -> None:
    ...
    ap.add_argument("--workers",       type=int,   default=8,
                    help="Parallel workers for ESPN game summary fetches (default 8)")
    args = ap.parse_args()
    ...
    out_cols  = stat_cols + [
        ...
        "min_last5_avg",  # actual minutes played avg — used by step6 for minutes_tier
    ...
    for idx, row in slate.iterrows():
        ...
        # Compute minutes avg separately from the prop stat — always use MIN column
        if not is_combo:
            min_mask = (
                (cache["SEASON"].astype(str) == str(args.season)) &
                (cache["ESPN_ATHLETE_ID"].astype(str) == str(aid))
            ...
            if cutoff_date is not None:
                min_mask = min_mask & (cache["GAME_DATE"] >= cutoff_date)
            min_vals = pd.to_numeric(
                cache.loc[min_mask].sort_values("GAME_DATE", ascending=False)["MIN"],
                errors="coerce"
            ).dropna().tolist()
            if min_vals:
                slate.at[idx, "min_last5_avg"] = fmt_num(float(np.mean(min_vals[:5])))
    ...
    if map_updates > 0:
        ...
        df_map.to_csv(idmap_path, index=False, encoding="utf-8-sig")
    ...
    if args.debug_misses and misses_rows:
        ...
        miss_df.to_csv(args.debug_misses, index=False, encoding="utf-8-sig")
