            ...
            for athlete in stat_group.get('athletes', []):
                ...
                for label, val in zip(labels, stats):
                    ...
                    try:
                        ...
                        # keep non-numeric in raw_map for parsing (e.g., '7-12')
                        pass

                # Parse made-attempt strings if ESPN uses 'FG', '3PT', 'FT', '2PT'
                ...
                # Common ESPN labels
                fg_m, fg_a = _parse_made_att(raw_map.get('FG') or raw_map.get('FGM-A') or raw_map.get('FGMA'))
                if fg_m is not None and fg_a is not None:
                    stat_map['FGM'] = fg_m
                    stat_map['FGA'] = fg_a

                t3_m, t3_a = _parse_made_att(raw_map.get('3PT') or raw_map.get('3FG') or raw_map.get('3PTM-A'))
                if t3_m is not None and t3_a is not None:
                    stat_map['3PM'] = t3_m
                    stat_map['3PA'] = t3_a

                ft_m, ft_a = _parse_made_att(raw_map.get('FT') or raw_map.get('FTM-A'))
                if ft_m is not None and ft_a is not None:
                    stat_map['FTM'] = ft_m
                    stat_map['FTA'] = ft_a

                tw_m, tw_a = _parse_made_att(raw_map.get('2PT') or raw_map.get('2FG') or raw_map.get('2PTM-A'))
                if tw_m is not None and tw_a is not None:
                    stat_map['2PM'] = tw_m
                    stat_map['2PA'] = tw_a
...
def fetch_events_for_date(sport_path, date_str):
    """Fetch all event dicts from ESPN scoreboard for a given date string."""
    ...
    try:
        ...
        print(f"  WARNING: scoreboard fetch failed for {date_str}: {e}")
        return []
...
def fetch_sport(sport_path, date_str):
    ...
    # NBA: ESPN dates games reliably by local tip-off date — single date fetch is safe.
    # CBB: ESPN frequently indexes college games under an adjacent calendar date due to
    # timezone handling and conference scheduling. Use a 3-day window (day-1, day, day+1)
    # to capture all completed games, then deduplicate by event_id.
    ...
    if is_cbb:
        ...
        print(f"CBB mode: fetching 3-day window ({fetch_dates[0]} → {fetch_dates[2]}) to capture all games for {date_str}")
    else:
    ...
    for d in fetch_dates:
        ...
        day_events = fetch_events_for_date(sport_path, d)
        new = 0
        for e in day_events:
            ...
            if eid and eid not in seen_ids:
                ...
                events.append(e)
                new += 1
        print(f"  Found {len(day_events)} games ({new} new unique)")

    print(f"  Total unique events to check: {len(events)}")
    ...
    # Deduplicate per player+team+prop — keep highest actual value
    # (guards against a player appearing on both date pages)
    df['actual'] = pd.to_numeric(df['actual'], errors='coerce')
    df = df.sort_values('actual', ascending=False).drop_duplicates(
        subset=['player', 'team', 'prop_type'], keep='first'
    )
