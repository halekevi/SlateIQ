...
if ($NHLOnly) {
    ...
    # NHL Step 9 disabled -- not needed
    # if ($ok) { $ok = Run-Step "NHL Step 9 - Build Tickets" $NHLDir ".\\scripts\\step9_build_tickets_nhl.py" "--input outputs\\step8_nhl_direction_clean.xlsx --output outputs\\step9_nhl_tickets.xlsx" }
    Write-Host ""
...
if ($NBAOnly) {
    ...
    if ($ok) { $ok = Run-Step "NBA Step 6d - H2H Matchup Stats"      $NBADir ".\\scripts\\step6d_attach_h2h_matchups.py"            "--input data\\outputs\\step6c_with_schedule_flags.csv --cache nba_espn_boxscore_cache.csv --output data\\outputs\\step6d_with_h2h.csv" }
...
if ($CBBOnly) {
    ...
    if ($ok) { $ok = Run-Step "CBB Step 6 - Rank Props"              $CBBDir ".\\scripts\\pipeline\\step6_rank_props.py"                  "--input step5b_cbb.csv --output step6_ranked_cbb.xlsx" }
    Write-Host ""
...
if ($CombinedOnly) {
    ...
    $NBAJob = Start-Job -ScriptBlock {
        ...
        if ($ok) { $ok = Run-Step-Job "NBA Step 6d - H2H Matchup Stats"      $NBADir ".\\scripts\\step6d_attach_h2h_matchups.py"            "--input data\\outputs\\step6c_with_schedule_flags.csv --cache nba_espn_boxscore_cache.csv --output data\\outputs\\step6d_with_h2h.csv" }
    ...
    $CBBJob = Start-Job -ScriptBlock {
        ...
        if ($ok) { $ok = Run-Step-Job "CBB Step 6 - Rank Props"              $CBBDir ".\\scripts\\pipeline\\step6_rank_props.py"                  "--input step5b_cbb.csv --output step6_ranked_cbb.xlsx" }
        return $ok
    ...
    if ($IncludeNHL) {
        $NHLJob = Start-Job -ScriptBlock {
            ...
            # NHL Step 9 disabled -- not needed
            # if ($ok) { $ok = Run-Step-Job "NHL Step 9 - Build Tickets" $NHLDir ".\\scripts\\step9_build_tickets_nhl.py" "--input outputs\\step8_nhl_direction_clean.xlsx --output outputs\\step9_nhl_tickets.xlsx" }
            return $ok
    ...
    $NBASuccess = Test-Path (Join-Path $NBADir "data\outputs\step8_all_direction_clean.xlsx")
    $CBBSuccess = Test-Path (Join-Path $CBBDir "step6_ranked_cbb.xlsx")
    if ($IncludeNHL)    { $NHLSuccess    = Test-Path (Join-Path $NHLDir    "outputs\step8_nhl_direction_clean.xlsx") }
    if ($IncludeMLB)    { $MLBSuccess    = Test-Path (Join-Path $MLBDir    "step8_mlb_direction_clean.xlsx") }
    if ($IncludeSoccer) { $SoccerSuccess = Test-Path (Join-Path $SoccerDir "outputs\step8_soccer_direction_clean.xlsx") }
...
if ($NBASuccess -and $CBBSuccess -and -not $NBAOnly -and -not $CBBOnly -and -not $NHLOnly -and -not $MLBOnly -and -not $SoccerOnly -and -not $WNBAOnly) {
    ...
    Push-Location $Root
    try {
        git add "ui_runner/templates/tickets_latest.html" "ui_runner/templates/tickets_latest.json" "ui_runner/templates/slate_eval_$Date.html" "ui_runner/templates/ticket_eval_$Date.html" 2>&1 | Out-Null
