...
def load_cbb(path: str) -> pd.DataFrame:
    ...
    # Keep columns as-is; just ensure the core fields exist for grading/summary
    df = _alias_cols(df)

    # CBB slate files (from your pipeline) already contain: player, prop_type, line, pick_type,
    # final_bet_direction / bet_direction, tier, edge, etc. If prop_type_norm missing, create it.
    if "prop_type_norm" not in df.columns:
        df["prop_type_norm"] = df.get("prop_type", "")
    if "bet_direction" not in df.columns and "final_bet_direction" in df.columns:
        df["bet_direction"] = df["final_bet_direction"]
...
def apply_actuals(df, actuals_path):
    ...
    act["player_key"] = act["player"].astype(str).str.lower().str.strip() + "|" + act["prop_type"].apply(norm_prop_key)
    ...
    for key in df["player_key"]:
        ...
        if key not in act_map and prop_part not in {k.split("|", 1)[-1] for k in act_map}:
    ...
    for _, row in df.iterrows():
        ...
        if pd.isna(actual):
            ...
            if parts:
                base_prop = norm_prop_key(row.get("prop_type_norm", ""))
                vals = []
                ok = True
                for p in parts:
        ...
        if pd.notna(void_r) and str(void_r).strip() not in ("", "nan"):
            results.append("VOID")
            void_reasons.append(void_r)
