...
- Fix edge_adj_dr to be direction-aware: UNDERs now get a positive edge
  contribution when projection < line (was previously always penalized).
- Support projection building for volume props so they don't VOID:
  2PTA/2PTM, 3PTA/3PTM, FTA/FTM (and also FGA/FGM).
- Adds prop_norm aliases (2pta, 2ptm, 3pta, 3ptm, fta, ftm, etc.)
- Adds projection fallback using *_player_last5_avg/last10_avg/season_avg
  when stat_last5_avg etc are blank for those props.
"""
...
_PROP_HIT_RATE_PRIOR = {
    ...
    "pf":                    0.424,
    "personalfouls":         0.424,
    # reasonable defaults for FT volume props if not present in your empirical table:
    "fta":                   0.545,
...
# -------------------- NEW: projection fallback --------------------

# Map prop_norm -> the stat prefix used for player-level avg columns created upstream (step4)
# Ex: fg2a -> fg2a_player_last5_avg, etc.
...
def _get_player_avg(row: pd.Series, base_prefix: str, window: str) -> float:
    """
    Try common column name variants:
      {prefix}_player_last5_avg / {prefix}_player_last10_avg / {prefix}_player_season_avg
      {prefix}_last5_avg        / {prefix}_last10_avg        / {prefix}_season_avg
    """
    candidates = []
    if window == "last5":
        candidates = [f"{base_prefix}_player_last5_avg", f"{base_prefix}_last5_avg"]
    elif window == "last10":
        candidates = [f"{base_prefix}_player_last10_avg", f"{base_prefix}_last10_avg"]
    elif window == "season":
        candidates = [f"{base_prefix}_player_season_avg", f"{base_prefix}_season_avg"]
    for c in candidates:
        if c in row.index:
            v = _safe_float(row.get(c))
            if not np.isnan(v):
...
def _projection_from_row(row: pd.Series) -> float:
    """
    Primary: weighted blend from stat_last5_avg / stat_last10_avg / stat_season_avg (50/30/20).
    Fallback (for volume props): use *_player_* averages if stat_* are missing.
    """
    prop_norm = str(row.get("prop_norm", "")).lower().strip()

    # 1) Try the normal stat_* averages
    ...
    # 2) If missing, try player-level stat averages for supported volume props
    if total_w < 0.1:
        base_prefix = _PLAYER_PREFIX_BY_PROP.get(prop_norm)
        if base_prefix:
            v5 = _get_player_avg(row, base_prefix, "last5")
            v10 = _get_player_avg(row, base_prefix, "last10")
            vs = _get_player_avg(row, base_prefix, "season")
            for v, w in [(v5, 0.50), (v10, 0.30), (vs, 0.20)]:
                if not np.isnan(v):
                    total_v += v * w
                    total_w += w
    ...
    # combo & fantasy corrections (keep existing behavior)
    corrections = {
        "pr":      1.05,
        "pa":      1.06,
        "ra":      1.08,
    ...
    return raw * corrections.get(prop_norm, 1.0)
...
def main() -> None:
    ...
    # Normalize prop names -> canonical keys (expanded to include 2PTA/2PTM/3PTA/3PTM/FTA/FTM)
    _PROP_NORM_MAP = {
        # 3pt
        "3-pt made":                 "fg3m",
        "3-pt attempted":            "fg3a",
    ...
    # Build projection (now includes volume-prop fallback)
    ...
    # Direction-aware: for UNDERs, projection < line is the favourable case,
    # so flip the sign before transforming so correct UNDERs get a positive contribution.
